...
class MulticlassClassifier(BaseEstimator, RegressorMixin):
    ...
    def predict(self, X, y=None):
        ...
        for j in range(num_instances):
            preds.append(self.graph.get_prediction(
                input_values={"x": X[j]}).reshape(1, -1))
...
def main():
    ...
    np.random.seed(2)
    X, y = make_blobs(n_samples=500, cluster_std=.25,
    ...
    test_acc = np.sum(np.argmax(estimator.predict(
        test_X), axis=1) == test_y)/len(test_y)
